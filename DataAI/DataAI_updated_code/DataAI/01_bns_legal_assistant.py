...
# Simplest possible app - no complex strings
dbutils.fs.put("/Workspace/Users/da25b048@smail.iitm.ac.in/DataAI/app.py", """
import streamlit as st
...
if prompt:
    ...
    if not answer:
        answer = "Try asking about: rape, theft, murder, or gang rape."
...
dbutils.fs.put("/Workspace/Users/da25b048@smail.iitm.ac.in/DataAI/requirements.txt", "streamlit", overwrite=True)
print("✅ requirements.txt created")

# COMMAND ----------

# Get your real BNS data
import pandas as pd
import json

sections_df = spark.table("bricksiitm.bns_legal.bns_sections").toPandas()

# Create weighted keyword mapping for better search
real_data = []
for _, row in sections_df.iterrows():
    real_data.append({
        "section": int(row['section_number']),
        "text": row['section_text'][:1000]
    })

print(f"Loaded {len(real_data)} real BNS sections")

# Create app with improved search
app_code = '''import streamlit as st
import re
...
# Real BNS data loaded from your database
BNS_DATA = ''' + json.dumps(real_data) + '''

def search_bns(question):
    """Smart search for BNS sections"""
    question_lower = question.lower()
    
    # Define keywords for different crimes
    crime_keywords = {
        "theft": ["theft", "steal", "stolen", "thief"],
        "robbery": ["robbery", "rob", "robbed", "robber"],
        "murder": ["murder", "kill", "killed", "homicide", "death"],
        "rape": ["rape", "raped", "sexual assault"],
        "kidnapping": ["kidnap", "kidnapping", "abduct", "abduction"],
        "cheating": ["cheat", "cheating", "fraud"],
        "dacoity": ["dacoity", "dacoit"],
        "extortion": ["extortion", "extort"]
    }
    
    # Find which crime is being asked about
    crime_type = None
    ...
    # Also check for punishment questions
    looking_for_punishment = any(word in question_lower for word in ["punishment", "penalty", "sentence", "jail", "prison"])
    ...
    for item in BNS_DATA:
        ...
        # If we identified crime type, look for sections about that crime
        if crime_type:
            if crime_type in text_lower:
                score += 10
            
            # Check for specific keywords in the text
            for kw in crime_keywords.get(crime_type, []):
                if kw in text_lower:
                    score += 5
        
        # If asking about punishment, look for punishment keywords
        if looking_for_punishment:
            if "punish" in text_lower or "imprisonment" in text_lower or "fine" in text_lower:
                score += 3
        
        # Check if the section number is small (earlier sections often definitions)
        if item["section"] < 100 and crime_type:
            score += 2
        
        # Check question words in text
...
if "messages" not in st.session_state:
    st.session_state.messages = [
        {"role": "assistant", "content": "Hello! Ask me about BNS - theft, robbery, murder, rape, kidnapping, cheating. What would you like to know?"}
    ]

for msg in st.session_state.messages:
    st.chat_message(msg["role"]).write(msg["content"])

prompt = st.chat_input("Ask a question about BNS...")

if prompt:
    st.session_state.messages.append({"role": "user", "content": prompt})
    st.chat_message("user").write(prompt)
    
    result, score = search_bns(prompt)
    ...
    if result and score > 0:
        ...
        note = ""
        if score < 5:
            note = "\\n\\n*Note: This is the most relevant section found. You may want to rephrase your question.*"
        
        answer = f"**BNS Section {result['section']}**\\n\\n{result['text']}\\n\\n---\\n⚠️ For legal advice, consult a lawyer.{note}"
    else:
        answer = "I couldn't find a specific BNS section. Try asking about: theft, robbery, murder, rape, kidnapping, or cheating."
...
print(f"✅ Improved app created with {len(real_data)} real BNS sections")
...
# Find sections that contain theft definition
sections_df = spark.table("bricksiitm.bns_legal.bns_sections").toPandas()

print("Sections containing 'theft' in the title or definition:\n")
for _, row in sections_df.iterrows():
    text = row['section_text'].lower()
    if 'theft' in text and ('definition' in text or 'whoever' in text or 'section' in text):
        if row['section_number'] < 200:  # Likely theft section
            print(f"Section {row['section_number']}:")
            print(f"  {row['section_text'][:300]}...")
            print()

# COMMAND ----------

# Run this to create improved app
import pandas as pd
import json

sections_df = spark.table("bricksiitm.bns_legal.bns_sections").toPandas()

# Create curated list with better section mapping
bns_data = []
for _, row in sections_df.iterrows():
    section = int(row['section_number'])
    ...
    # Tag sections by crime type
    tags = []
    if 'theft' in text_lower and section < 320:
        tags.append('theft')
...
print(f"Loaded {len(bns_data)} sections with tags")

# Create app with tag-based search
app_code = '''import streamlit as st
...
# BNS Data with tags
BNS_DATA = ''' + json.dumps(bns_data) + '''

def search_bns(question):
    question_lower = question.lower()
    
    # Determine crime type from question
    crime_map = {
        "theft": ["theft", "steal", "stolen", "thief"],
        "robbery": ["robbery", "rob", "robbed"],
        "murder": ["murder", "kill", "homicide", "death"],
    ...
    for item in BNS_DATA:
        ...
        # If crime detected, prioritize sections with that tag
        if detected_crime:
            if detected_crime in item.get("tags", []):
                score += 50
        
        # Check if text contains the crime word
        text_lower = item["text"].lower()
        if detected_crime and detected_crime in text_lower:
            score += 20
        
        # Smaller section numbers often have definitions
        if item["section"] < 100 and detected_crime:
            score += 5
    ...
    # If no match by tag, try keyword search
...
if "messages" not in st.session_state:
    st.session_state.messages = [
        {"role": "assistant", "content": "Ask me about: theft, robbery, murder, rape, cheating. What would you like to know?"}
    ]
...
if prompt:
    ...
    if result and score > 0:
        # Get only first 400 chars of the relevant section
        display_text = result["text"][:450]
        if len(result["text"]) > 450:
            display_text += "..."
        
        answer = f"**BNS Section {result['section']}**\\n\\n{display_text}\\n\\n---\\n⚠️ For legal advice, consult a lawyer."
    else:
        answer = "Try asking: what is theft? what is murder? punishment for rape?"
...
print("✅ Improved app created")

# COMMAND ----------

# Create a curated mapping (based on actual BNS structure)
# You can update these section numbers after verifying your PDF

crime_mapping = {
    "theft": {
        "section": 303,
        "text": "Theft: Whoever dishonestly takes movable property without consent is punished with imprisonment up to 3 years or fine or both."
    },
    "robbery": {
        "section": 310,
        "text": "Robbery: Theft or extortion involving fear of instant death or hurt is robbery. Punishment: imprisonment up to 10 years and fine."
    },
    "murder": {
        "section": 101,
        "text": "Murder: Causing death with intention to cause death. Punishment: death or imprisonment for life and fine."
    },
    "rape": {
        "section": 64,
        "text": "Rape: Non-consensual sexual intercourse. Punishment: rigorous imprisonment not less than 10 years, up to life imprisonment."
    },
    "gang rape": {
        "section": 70,
        "text": "Gang Rape: Rape by one or more persons in a group. Punishment: rigorous imprisonment not less than 20 years, up to life imprisonment."
    },
    "cheating": {
        "section": 318,
        "text": "Cheating: Fraudulently deceiving someone to deliver property. Punishment: imprisonment up to 3 years or fine or both."
    },
    "kidnapping": {
        "section": 137,
        "text": "Kidnapping: Taking or enticing a person without consent. Punishment: imprisonment up to 7 years and fine."
    }
}

# Save as JSON
import json
json_str = json.dumps(crime_mapping)

# Create new app with curated responses
...
# Curated BNS crime mapping
BNS_DATA = {json_str}

def get_answer(question):
    question_lower = question.lower()
    
    # Check which crime is being asked
    for crime, info in BNS_DATA.items():
        if crime in question_lower:
            return info
    
    # Check for punishment specific questions
    if "punishment" in question_lower:
        for crime, info in BNS_DATA.items():
            if crime in question_lower:
                return {{
                    "section": info["section"],
                    "text": f"Punishment for {crime}: " + info["text"].split("Punishment:")[-1].strip()
                }}
...
if "messages" not in st.session_state:
    st.session_state.messages = [
        {{"role": "assistant", "content": "Ask me about: theft, robbery, murder, rape, gang rape, cheating, kidnapping. What would you like to know?"}}
    ]
...
print("✅ Curated app created with clean answers")

# COMMAND ----------

# Restore the simple working app
dbutils.fs.put("/Workspace/Users/da25b048@smail.iitm.ac.in/DataAI/app.py", """
import streamlit as st
...
print("✅ Working hardcoded app restored")
